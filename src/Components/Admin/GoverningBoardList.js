...
function GoverningBoardList({
                ...
                myData.push({
                    ...
                    status: (
                        <label class="switch">
                            <input
                                type="checkbox"
                                checked={parseInt(data.status) ? true : false}
                                onChange={() => {
                                    updateStatus(
                                        data.id,
                                        data.status ? 0 : 1,
    ...
    async function getOfficers(setList, page) {
        await API.get("/boards", { params: { page } })
            .then((res) => {
    ...
    async function updateStatus(id, status, setShow, setMsg, list, setList) {
        await API.post("/boards/disable", { id: id, status: status })
            .then((res) => {
                if (res.data.status === "success") {
                    setList(pre => ({
                        ...pre,
...
export default GoverningBoardList;
